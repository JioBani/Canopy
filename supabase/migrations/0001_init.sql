...
-- ur_group — UR 묶음 (기능 소유, 가독성용)
create table ur_group (
  id          uuid primary key default gen_random_uuid(),
  feature_id  uuid not null references node(id) on delete cascade,   -- 기능 노드
  name        text not null,
  sort_order  int  not null default 0,
  created_at  timestamptz not null default now(),
  updated_at  timestamptz not null default now()
...
--     ur_group.feature_id, ur.feature_id 는 기능 노드여야 한다.
--     ur_work_link.work_id 는 작업 노드여야 한다.
-- ---------------------------------------------------------------------------
create or replace function check_node_is_feature()
returns trigger
language plpgsql
as $$
begin
  if (select type from node where id = new.feature_id) is distinct from '기능' then
    raise exception 'feature_id(%)는 기능 노드여야 합니다.', new.feature_id;
  end if;
  return new;
end;
$$;
...
create trigger trg_ur_group_feature_check
  ...
  for each row execute function check_node_is_feature();
